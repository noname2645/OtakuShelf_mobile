...
// ─── InputField ───────────────────────────────────────────────────────────────
const InputField = ({ icon, placeholder, value, onChangeText, secureTextEntry, keyboardType, autoCapitalize, editable, maxLength, textAlign, fontSize, letterSpacing }) => {
...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Normalise response — backend may use res.data.user or res.data.data.user
...
const styles = StyleSheet.create({
  ...
  orb1: {
    ...
    // blur workaround — use opacity + large radius
  },
  orb2: {
  ...
  floatingEmoji: {
    position: 'absolute', fontSize: 26, opacity: 0.18,
  },
  ...
  // Primary button
  primaryBtn: { borderRadius: 50, overflow: 'hidden', marginTop: 6, marginBottom: 4 },
  primaryBtnInner: {
